(() => {
  let savedPrompts = [];
  let prompterPanelExists = false; // Track whether the panel is already appended
  ...
  const Prompter = async () => {
    ...
    // Check if prompterPanel already exists in the DOM
    const existingPanel = document.querySelector(".prompter\\.abz4375");
    if (existingPanel || prompterPanelExists) {
      // If it exists, remove it or avoid creating a duplicate
  ...
  const pushPrompt = (prompt, promptStack, index) => {
    ...
    textarea.className =
      "w-full bg-transparent text-white border-none outline-none text-sm resize-none";

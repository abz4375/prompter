(async () => {
  ...
  const initializePrompter = async () => {
    ...
    const pushPrompt = (prompt) => {
      ...
      promptTitle.style =
        "border:none;background-color: black; resize:none; outline:none; width: 120px";
      ...
      saveBtn.onclick = async () => {
        if (!(promptTitle.value === `` && promptContent.innerText === ``)) {
          ...
          saveThePrompt({
            ...
            pContent: "" + promptContent.innerText,
          });
  ...
  if (pageAccessedByReload) {
    if (document.getElementById("prompter.abz4375.started")) {
    } else {
      window.open("https://bard.google.com/", "_self");
    }
  }
})();
